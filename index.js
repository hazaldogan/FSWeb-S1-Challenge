...
function indekseGoreFenomen(arr,ind) {
  let profile = arr[ind].profile
  return `${ind}. indekste bulunan fenomen: ${profile}`
}
...
function fenomenEkle(arr,no,profil,follower,post,platf) {
  arr.push({
    number: no,
    profile:profil,
    followers:follower,
...
function enFenomenler(arr) {
  let newArr = [];
  for(let a of arr){
    if(a.followers > 100000000){
      newArr.push(a.profile);
    }
  }
  return newArr
}
...
function platformaGoreCokGonderiYapanFenomen(arr,platf){
  let max = 0;
  let fenomen = "";
  for(let a of arr){
    if(a.posts > max && a.platform === platf){
      max = a.posts;
      fenomen = a.profile;
    }
  }
